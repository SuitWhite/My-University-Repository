    ...
    while ch != 7:
        ...
        if ch == 2:
            ...
            if left > right:
                ...
                left = right
                right = temp
            choice = input("Enter 0 for Iterator, else for generator: ")
            generator_type = Iterator(number, left, right) if choice == 0 else generator(number, left, right)
            container.extend(generator_type)
